...
const buttonColorClasses = computed(() => {
    if (props.active) {
        ...
        return 'bg-accent hover:bg-accent-400/70 focus:bg-accent';
    }
});
...
<template>
    <button data-testid="timer_button" :class="twMerge(
        ...
        <Transition name="fade" mode="out-in">
            <svg v-if="props.active" :class="iconClass[size]" viewBox="0 0 14 14" fill="none"
            ...
            <svg v-else :class="iconClass[size]" viewBox="0 0 7 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" clip-rule="evenodd"
